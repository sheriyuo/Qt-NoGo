...
\begin{document}
	...
	\begin{figure}[!htb]{
			\centering
			\includegraphics[width=0.8\textwidth]{img/web1.png}
			\counterwithin{figure}{subsection}
			\caption{联机设置页面}
		}
	\end{figure}
   ...
   \begin{figure}[!htb]
    	\centering
    	\includegraphics[width=6cm]{img/web4.png}
    	\includegraphics[width=6cm]{img/web5.png}
    	\includegraphics[width=6cm]{img/web2.png}
    	\includegraphics[width=6cm]{img/web3.png}
    	\counterwithin{figure}{subsection}
    	\caption{联机对局界面演示}
    \end{figure}
    ...
    当一方认输，即点击 Resign 按钮后，认输方发送 \verb|GIVE_UP_OP|，胜方在收到该信息后回复 \verb|GIVE_UO_END_OP| 表示请求已“一方认输”为理由结束对局。认输方收到 \verb|GIVE_UO_END_OP| 回复同样的信息以确认。此时联机对战结束，棋盘不能被操作。
    
    当一方超时，胜方的计时器会调用 \verb|GameWidget::playerTimeout_OL()| 槽函数，发送 \verb|TIMEOUT_END_OP| 表示请求已“一方超时”为理由结束对局，负方回复同样的信息以确认。此时联机对战结束，棋盘不能被操作。
