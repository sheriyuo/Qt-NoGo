...
public slots:
    ...
    virtual void on_resignButton_clicked(); // 链接 resign 按钮的行为 (offline)
    virtual void on_restartButton_clicked(); // 链接 restart 按钮的行为 (offline)

    void startTimer();    // 当对局开始时，无论谁先手（机器下第一个棋子认为不需要时间），都打开玩家的计时器
    virtual void playerTimeout(); // 玩家超时的槽函数，用于链接计时器 (offline)
...
protected:
    ...
    virtual void takeTurn(int row, int col); // 轮流下棋
    ...
    */
    virtual void sendMessage(int type);
    ...
    // 玩家的计时器 将 bot 的计时器内置于 class Bot 中
    QTimer *timerForPlayer;
    QTimer *timerForBar;
    time_t basetime;

    // 界面绘制
    int deltaY, LOGO_X, LOGO_Y, logoBoardW, logoBoardH;
